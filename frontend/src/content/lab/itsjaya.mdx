...
tech: [Next.js 16, React 19, Tailwind CSS v4, TypeScript, FastAPI, ChromaDB, Gemini 2.5 Flash, BAAI/bge-base-en-v1.5, HyDE, BM25, RRF, Knowledge Graph, FNV-1a Hash, SQLite, SlowAPI, AWS Lightsail, Nginx, S3, GitHub Pages, GitHub Actions]
links:
  - label: GitHub
...
│  POST /admin/reingest     force re-embed (ADMIN_TOKEN)                   │
...
│  │  7-day timestamped       │   │  Writes auto-regenerate JSON +      │ │
│  │  + latest_analytics.db   │   │  trigger run_ingest() in BG.        │ │
│  └──────────────────────────┘   └──────────────────────────────────────┘ │
│                                                                          │
│  ┌──────────────────────────────────────────────────────────────────┐   │
...
1. Calls the matching `regenerate_*_json()` function (`regenerate_blog_json()`, `regenerate_lab_json()`, or `regenerate_quotes_json()`) to sync content.db → JSON in `backend/data/knowledge/`
2. Calls `run_ingest()` — only changed documents are re-embedded (incremental ingest), zero user-visible latency

`run_ingest()` itself also calls `_sync_content_db()` at startup (lazy-imports all three regenerate functions), so the JSON files are always current before the hash diff runs.
...
<Stack items={["Next.js 16 (App Router)", "React 19", "Tailwind CSS v4", "TypeScript", "MDX + next-mdx-remote", "gray-matter", "FastAPI", "Python 3.11", "Pydantic Settings", "SlowAPI (rate limiting)", "ChromaDB", "fastembed (BAAI/bge-base-en-v1.5 ONNX, 768-dim)", "rank-bm25 (BM25Okapi)", "HyDE (Hypothetical Document Embeddings)", "Knowledge Graph (static, in-memory)", "google-genai SDK", "Gemini 2.5 Flash", "SQLite3 (analytics.db)", "SQLite3 (content.db)", "AWS Lightsail 2GB", "Nginx + Let's Encrypt", "Amazon S3", "Docker", "GitHub Pages", "GitHub Actions"]} />
