...
tech: [Next.js 16, FastAPI, ChromaDB, Gemini 2.5 Flash, BAAI/bge-base-en-v1.5, BM25, Knowledge Graph, SQLite, AWS Lightsail, Nginx, S3, GitHub Pages, GitHub Actions]
links:
  - label: GitHub
...
│  │  bge-base-en-v1.5 (768-dim)│   │  interactions                    │  │
│  │  LRU cache (256 entries)   │   │  ├── ip_hash  TEXT              │  │
│  │  Knowledge Graph (static)  │   │  └── created_at  TIMESTAMP      │  │
│  │  BM25Okapi  (rank_bm25)    │   │                                  │  │
...
│                      │   │  "3000 RPS", "SnapLog",      │
│  bge-base-en-v1.5    │   │  "Qualcomm", "78%",          │
│  768-dim ONNX        │   │  "115 GB/day"                │
│  HNSW cosine         │   │                              │
...
<Stack items={["Next.js 16 (App Router)", "React 19", "Tailwind CSS v4", "TypeScript", "MDX + next-mdx-remote", "gray-matter", "FastAPI", "Python 3.11", "Pydantic Settings", "ChromaDB", "fastembed (BAAI/bge-base-en-v1.5 ONNX, 768-dim)", "rank-bm25 (BM25Okapi)", "Knowledge Graph (static, in-memory)", "google-genai SDK", "Gemini 2.5 Flash", "SQLite3", "AWS Lightsail 2GB", "Nginx + Let's Encrypt", "Amazon S3", "Docker", "GitHub Pages", "GitHub Actions"]} />
...
<Decision date="2026-05-22" title="BAAI/bge-base-en-v1.5 over all-MiniLM-L6-v2 — better retrieval, same ONNX runtime">
  ...
  The switch stays fully ONNX via fastembed — no PyTorch dependency, no RAM regression beyond ~30–50MB for the larger 768-dim model. The ONNX binary is ~90MB vs 45MB, a one-time download. Critically, switching embedding models requires wiping ChromaDB and rebuilding from scratch because stored 384-dim vectors are incompatible with 768-dim queries. The auto-wipe mechanism (see next decision) handles this transparently on deploy.

  PyTorch was explicitly evaluated and rejected: PyTorch and ONNX produce byte-identical embeddings for the same model weights. Switching runtimes without upgrading the model changes nothing except adding ~600MB RAM and a PyTorch dependency. What matters is the model, not the runtime.
</Decision>

<Decision date="2026-05-22" title="Auto-wipe ChromaDB on embedding model change — zero manual steps on deploy">
  Changing the embedding model from 384-dim to 768-dim makes every stored vector incompatible. A naive deploy would start up, try to query with 768-dim vectors against a 384-dim collection, and crash with `InvalidArgumentError: Collection expecting dimension of 384, got 768`. The fallback kicks in but every query fails.

  The fix: `run_ingest()` stores the current `EMBED_MODEL` string under `__embed_model__` in `.doc_hashes.json`. On startup, if the stored model name differs from the constant in `store.py` (or is absent — which happens when hashes exist from before tracking was added), `reset_collection()` is called first and `force=True` is set. ChromaDB wipes cleanly and all 87 documents are re-embedded with the new model before the app starts serving traffic. No environment variable, no manual SSH, no pre-deploy migration step required.

  The absence case (`stored_model is None`) deliberately triggers a wipe rather than assuming the existing collection is compatible. On a fresh instance with no hashes file, the collection is also empty so the wipe is a no-op. On an existing instance with an old hashes file, the wipe is exactly what's needed.
</Decision>
...
<Decision date="2026-05-22" title="Static knowledge graph for context expansion — no graph library, no LLM extraction">
  ...
  Full GraphRAG (Microsoft's approach using community detection and LLM-extracted entities) was evaluated and rejected. It's designed for corpora with millions of nodes. For 87 documents with completely known structure, the extraction step is pure overhead — we already know the relationships from the JSON files.

  The implementation is a static in-memory graph built at startup from the knowledge JSON. Three relationship maps: `project → skill category docs` (from `projects.json tags[]`), `experience → project docs` (hard-coded temporal co-occurrence for the 5–6 known links), `project → experience doc` (reverse). After the RRF top-5 is selected, `expand_context()` follows 1-hop edges and pulls in at most 2 related docs from the RRF-20 pool — no extra ChromaDB call, zero added latency. The expanded docs go through the same `_build_context()` grouping as the retrieved ones.

  The hard-coded experience→project links are a deliberate choice over dynamic temporal overlap detection. The temporal heuristic produces false positives (the portfolio project spans all NYU time, so it incorrectly maps to every NYU experience). A 6-line dict with the real mappings is correct, readable, and maintainable.
</Decision>
...
<Update date="2026-05-22">
  Added auto-wipe on embedding model change. run_ingest() now stores the current EMBED_MODEL in .doc_hashes.json. If the stored model name differs on startup, reset_collection() is called automatically and all 87 docs are re-ingested with the new model. Handles both explicit model changes and the first-startup case where no model was previously tracked.
</Update>

<Update date="2026-05-22">
  Added static knowledge graph (graph.py). build_graph() constructs three relationship maps from knowledge JSON at startup: project→skills, experience→projects, project→experience. expand_context() does 1-hop traversal after RRF top-5 rerank, pulling up to 2 related docs from the RRF-20 pool into the Gemini context window. Zero extra ChromaDB calls, zero latency penalty. Both /ai/chat and /ai/chat/stream updated.
</Update>
...
- Cross-encoder reranking (`cross-encoder/ms-marco-MiniLM-L-6-v2`) — already scaffolded in `rerank_cross_encoder()`, gated on Lightsail upgrade to 4GB ($20/month) for the ~250MB RAM headroom
- Entity-aware ChromaDB pre-filtering using the metadata fields added this session (entity_type, future company_key, project_key)
