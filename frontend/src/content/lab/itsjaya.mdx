...
<Decision date="2026-04-15" title="Hash-based re-ingest: SHA-256 of all knowledge JSON on startup">
  ...
  SHA-256 of the entire `data/knowledge/` directory solves both failure modes with a single file read at startup. If the volume is wiped, the missing hash file triggers a fresh ingest, which is correct.
</Decision>
